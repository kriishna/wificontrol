 ...
 */
public abstract class WifiAdminNew {
    ...
    public WifiAdminNew(Context context) {
        ...
        MyLog.LogInfo(TAG, "getIpAddress = " + mWifiInfo.getIpAddress());
    }
    ...
    public void addNetwork(String ssid, String passwd, int type) {
        if (ssid == null || passwd == null || ssid.equals("")) {
            MyLog.LogInfo(TAG, "addNetwork() ## nullpointer error!");
            return;
        }

        if (type != TYPE_NO_PASSWD && type != TYPE_WEP && type != TYPE_WPA) {
            MyLog.LogInfo(TAG, "addNetwork() ## unknown type = " + type);
        }
    ...
    private synchronized void register() {
        MyLog.LogInfo(TAG, "register() ##mHaveRegister = " + mHaveRegister);
...
//      mTimer.schedule(mTimerTask, 0, 20 * 1000);// 20s
        mTimer.schedule(new mTimerTask(), 30 * 1000);
    }
    ...
    public int checkState(Context context) {
        WifiManager wifiManager = mWifiManager = (WifiManager) context.getSystemService(Context.WIFI_SERVICE);
